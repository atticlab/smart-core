...
TEST_CASE("Payment refund", "[tx][payment_refund]")
{
...
//	applySetOptions(app, commissionAccount, commissionSeq++, nullptr, nullptr, nullptr, nullptr, &signer, nullptr);
	...
	SECTION("Malformed payment reversal")
	{
		...
		{
			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, usd, -paymentAmount, paymentAmount, REFUND_INVALID_AMOUNT);
		}
		SECTION("Invalid asset")
		{
			Asset native;
			native.type(ASSET_TYPE_NATIVE);
			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, native, paymentAmount, paymentAmount, REFUND_INVALID_ASSET);
		}
	}
//	SECTION("Commission")
//	{
...
//			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, eur, paymentAmount, paymentAmount, REFUND_SRC_NO_TRUST);
//
//		}
		SECTION("Source line underfunded")
		{
			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, usd, paymentAmount + 1, paymentAmount + 1, REFUND_UNDERFUNDED);
		}
	}
	SECTION("Payment sender")
	{
		SECTION("Does not exists")
		{
			...
			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, randomAccount, usd, paymentAmount, paymentAmount, REFUND_NO_PAYMENT_SENDER);
		}
		SECTION("Line is full")
		{
			applyChangeTrust(app, account, root, accountSeq++, "USD", paymentAmount - 1);
			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, usd, paymentAmount, paymentAmount, REFUND_PAYMENT_SENDER_LINE_FULL);
		}
	}
	SECTION("Success")
	{
		...
		// apply refund
		applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, usd, paymentAmount - commissionAmount, paymentAmount);
		// check postbalances
//		commissionLine = loadTrustLine(commissionAccount, usd, app, true);
//		REQUIRE(commissionLine->getBalance() == 0);
		agentLine = loadTrustLine(agent, usd, app, true);
		REQUIRE(agentLine->getBalance() == commissionAmount);
		...
		{
			applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, usd, paymentAmount, paymentAmount, REFUND_ALREADY_REFUNDED);
		}
        SECTION("Another refund")
        {
            applyPaymentRefundOp(app, agent, agentSeq++, paymentID, account, usd,  commissionAmount, paymentAmount);
            agentLine = loadTrustLine(agent, usd, app, true);
